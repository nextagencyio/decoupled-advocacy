...
import { GET_ISSUE_BY_PATH } from '@/lib/queries'
import { DrupalIssue } from '@/lib/types'
...
interface IssueByPathData {
  route: {
    entity: DrupalIssue
  } | null
}

async function getIssue(path: string): Promise<DrupalIssue | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<IssueByPathData>({
      query: GET_ISSUE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/issues/${slug.join('/')}`
  const item = await getIssue(path)

  if (!item) {
    return { title: 'Issue Not Found | Advocacy' }
  }
