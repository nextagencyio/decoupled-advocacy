...
import { GET_REPORT_BY_PATH } from '@/lib/queries'
import { DrupalReport } from '@/lib/types'
...
interface ReportByPathData {
  route: {
    entity: DrupalReport
  } | null
}

async function getReport(path: string): Promise<DrupalReport | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ReportByPathData>({
      query: GET_REPORT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getReport(path)

  if (!item) {
    return { title: 'Report Not Found | Advocacy' }
  }
