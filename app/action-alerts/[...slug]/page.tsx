...
import { GET_ACTION_ALERT_BY_PATH } from '@/lib/queries'
...
interface ActionAlertByPathData {
  route: {
    entity: DrupalActionAlert
  } | null
}

async function getActionAlert(path: string): Promise<DrupalActionAlert | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ActionAlertByPathData>({
      query: GET_ACTION_ALERT_BY_PATH,
      variables: { path },
      fetchPolicy: 'cache-first',
    })
    return data?.route?.entity || null
  } catch (error) {
    console.error('Error fetching action alert:', error)
    return null
  }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/action-alerts/${slug.join('/')}`
  const item = await getActionAlert(path)

  if (!item) {
    return { title: 'Action Alert Not Found | Advocacy' }
  }

  return {
    title: `${item.title} | Advocacy`,
    description: ((item as any).body?.processed ? (item as any).body.processed.replace(/<[^>]*>/g, '').substring(0, 160) : '') || `Learn more about ${item.title}.`,
  }
}

export default async function ActionAlertDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/action-alerts/${slug.join('/')}`
  const item = await getActionAlert(path)
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).actionUrl && (
                    <div>
                      <dt className="text-sm text-gray-500">Action URL</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).actionUrl}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
