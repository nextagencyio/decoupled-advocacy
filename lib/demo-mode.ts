...
import actionAlertsData from '@/data/mock/action-alerts.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'issues.json': issuesData,
  'action-alerts.json': actionAlertsData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetIssues') || query.includes('nodeIssues')) {
      return loadMockData('issues.json')
    }

    if (query.includes('GetActionAlerts') || query.includes('nodeActionAlerts')) {
      return loadMockData('action-alerts.json')
    }

    if (query.includes('GetReports') || query.includes('nodeReports')) {
      return loadMockData('reports.json')
    }
